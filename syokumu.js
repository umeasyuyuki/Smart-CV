...
<head>
  ...
  <style>
    * {
    ...
    /* 右カラム */
    .right {
      /* もともと width: 50% としていましたが、
         左カラムを削除したため単独で全幅でもOKです。 */
      width: 100%;
      padding: 20px;
      overflow-y: auto;
      background: #9e9999;
    ...
    /* ========== 右カラム(プレビュー)やA4ページ固定部分は変更なし ========== */
    .resume-pages-wrap {
      /* ここに複数 .resume-page が追加される */
    }
    .resume-page {
      width: 210mm;
      height: 297mm;
...
<div class="container">
  ...
  <!-- 右カラム -->
  <div class="right">
    <div class="resume-pages-wrap" id="resumePages"></div>
  </div>
...
function setupBindings(){
  // ※ もともと左カラムにあった #input-* 各要素は削除されていますが、
  //   「他の部分はそのまま」という方針のため、JSコードだけは残しています。
  //   実行すると要素が見つからずエラーになる可能性があります。
...
// (3) 追加/削除ボタン (本来は左カラムの要素ですが、コードは残しています)
let careerCount = 1;
...
document.getElementById('remove-career-row')?.addEventListener('click', () => {
  if (careerCount > 1) {
    document.getElementById('career-extra')?.removeChild(
      document.getElementById('career-extra').lastElementChild
    );
    careerCount--;
  } else {
...
document.getElementById('remove-edu-row')?.addEventListener('click', () => {
  if (eduCount > 1) {
    document.getElementById('edu-extra')?.removeChild(
      document.getElementById('edu-extra').lastElementChild
    );
    eduCount--;
  } else {
...
document.getElementById('remove-license-row')?.addEventListener('click', () => {
  if (licenseCount > 1) {
    document.getElementById('license-extra')?.removeChild(
      document.getElementById('license-extra').lastElementChild
    );
    licenseCount--;
  } else {
...
document.getElementById('remove-lang-row')?.addEventListener('click', () => {
  if (langCount > 1) {
    document.getElementById('lang-extra')?.removeChild(
      document.getElementById('lang-extra').lastElementChild
    );
    langCount--;
  } else {
